...
class FileBrowserActivity : LearnsyFileManagerActivity() {
    ...
    private fun connectAndLoad() {
        ...
        lifecycleScope.launch {
            // Mỗi client (FTP/SFTP/SMB) đã tự đặt timeout riêng ở tầng socket (8-60s tuỳ
            // giao thức), nhưng nếu một trong các lệnh gọi thư viện bên dưới bị treo ở
            // chỗ nào đó không tôn trọng timeout đó (vd. DNS resolve, hoặc thư viện SMB
            // negotiate không phản hồi đúng như comment trong SmbClientManager mô tả),
            // trước đây không có giới hạn tổng nào ở tầng UI -> spinner xoay vô hạn,
            // người dùng không biết là đang chờ hay app đã treo, không có cách thoát ra
            // ngoài force-quit app. Bọc thêm 1 timeout tổng 45s ở đây làm lưới an toàn
            // cuối cùng: dù lỗi nằm ở đâu, UI luôn thoát khỏi trạng thái loading và báo
            // lỗi rõ ràng trong tối đa 45s thay vì treo vô thời hạn.
            ...
            when {
                result == null -> {
                    showLoading(false)
                    showError(getString(R.string.connect_timeout))
                }
                result.isSuccess -> {
                    loadCurrentFolder()
                    loadQuotaEstimate()
    ...
    private fun onFileClick(file: RemoteFile) {
        ...
        when {
            ext in AUDIO_EXTENSIONS -> playRemoteAudio(file)
            ext in VIDEO_EXTENSIONS -> playRemoteVideo(file)
            else -> downloadFile(file)
        }
    }

    /**
     * Phát audio trực tiếp — FTP dùng pipeline mới (LocalStreamServer, xem streamproxy/), SFTP/
     * SMB TẠM THỜI vẫn dùng pipeline cũ (RemoteMediaUri/FtpDataSource...) cho tới khi được
     * chuyển sang LocalStreamServer ở bước kế tiếp của quá trình viết lại (xem
     * streamproxy/sources/, chỉ mới có FtpRemoteSource). Không xoá pipeline cũ cho tới khi cả 3
     * giao thức đều đã chuyển xong, tránh SFTP/SMB bị gãy giữa chừng lúc đang refactor dở.
     */
    private fun playRemoteAudio(file: RemoteFile) {
        val url = if (profile.type == com.learnsypro.app.filemanager.model.ConnectionType.FTP) {
            val source = com.learnsypro.app.filemanager.streamproxy.sources.FtpRemoteSource(profile, file.path, file.name)
            com.learnsypro.app.filemanager.streamproxy.LocalStreamServer.getInstance().register(source)
        } else {
            com.learnsypro.app.filemanager.media.datasource.RemoteMediaUri.encode(profile, file.path).toString()
        }
    ...
    /** Phát video trực tiếp — cùng logic chọn pipeline với playRemoteAudio() ở trên. */
    private fun playRemoteVideo(file: RemoteFile) {
        val url = if (profile.type == com.learnsypro.app.filemanager.model.ConnectionType.FTP) {
            val source = com.learnsypro.app.filemanager.streamproxy.sources.FtpRemoteSource(profile, file.path, file.name)
            com.learnsypro.app.filemanager.streamproxy.LocalStreamServer.getInstance().register(source)
        } else {
    ...
    companion object {
        ...
        // Dùng để quyết định phát trực tiếp (streaming, không tải về đĩa) qua
        // playRemoteAudio()/playRemoteVideo() thay vì downloadFile() như các loại
        // file khác — xem giải thích chi tiết ở onFileClick() và RemoteMediaUri.kt.
        private val AUDIO_EXTENSIONS = setOf("mp3", "wav", "flac", "aac", "ogg", "m4a", "wma")
