...
class SftpClientManager : RemoteClient {
    ...
    override suspend fun connect(profile: FtpConnectionProfile): Result<Unit> = withContext(Dispatchers.IO) {
        try {
            ...
            // Timeout đọc socket cho TOÀN BỘ phiên SSH (bao gồm cả lúc truyền file
            // SFTP) — mặc định của SSHJ khá ngắn, không đủ cho file vài GB qua mạng
            // LAN chậm, dễ gây timeout/treo giữa chừng giống lỗi "Read timed out" đã
            // gặp ở FTP. keepAliveInterval bên dưới giữ kết nối sống, còn timeout này
            // là giới hạn tối đa cho MỖI lần đọc dữ liệu, không phải cho cả phiên.
