    ...
    override suspend fun connect(profile: FtpConnectionProfile): Result<Unit> = withContext(Dispatchers.IO) {
        try {
            ...
            val config = SmbConfig.builder()
                ...
                // LƯU Ý: cả 2 timeout này áp dụng cho MỌI request qua lại với server, kể
                // cả lúc đang đọc từng chunk của 1 file lớn đang tải — 8s cũ đủ để phát
                // hiện lỗi negotiate sớm nhưng lại quá ngắn khi thực sự đang truyền file
                // vài GB qua mạng LAN chậm (mỗi chunk đọc có thể mất hơn 8s), gây "Read
                // timed out"/TimeoutException giữa chừng dù kết nối vẫn bình thường.
                // Nâng lên 60s để đủ dùng cho cả 2 trường hợp.
