...
class StorageManagerActivity : LearnsyFileManagerActivity() {
    ...
    private fun populateActionList() {
        ...
        for ((index, row) in rows.withIndex()) {
            ...
            // TRƯỚC ĐÂY: gán cứng "0 B" cho cả 4 dòng và chỉ dòng Thùng rác (index 0) được
            // tính lại đúng số liệu thật bên dưới — 3 dòng còn lại (Ứng dụng không dùng,
            // File trùng lặp, File lớn) không có bất kỳ code nào tính lại nên MÃI MÃI hiện
            // "0 B" dù thực tế có dữ liệu. Đặt "..." thay vì "0 B" khi đang tính, để không
            // gây hiểu lầm là "đã tính xong, không có gì" trong lúc quét.
            itemBinding.tvActionSize.text = "…"
        ...
        /** Cập nhật đúng số liệu vào dòng thứ [index] (0=Thùng rác, 1=Ứng dụng không dùng, 2=File trùng lặp, 3=File lớn). */
        fun setActionSize(index: Int, sizeText: String) {
            if (index >= binding.actionList.childCount) return
            val row = binding.actionList.getChildAt(index)
            row.findViewById<android.widget.TextView>(R.id.tv_action_size)?.text = sizeText
        }
        ...
        lifecycleScope.launch {
            ...
            setActionSize(0, formatSize(trashSize))
        }

        // ĐÃ THÊM: 3 dòng còn lại giờ tính đúng dung lượng thật, chạy song song trên IO
        // thread để không chặn UI. Dùng ước lượng nhẹ (không hash MD5 đầy đủ như màn con
        // File trùng lặp) — đủ nhanh cho màn tổng quan, số chính xác tuyệt đối chỉ cần
        // khi người dùng bấm vào xem chi tiết từng màn.
        lifecycleScope.launch {
            val unusedAppsSize = withContext(Dispatchers.IO) { estimateUnusedAppsSize() }
            setActionSize(1, formatSize(unusedAppsSize))
        }
        lifecycleScope.launch {
            val duplicateReclaimable = withContext(Dispatchers.IO) { estimateDuplicateReclaimableSize() }
            setActionSize(2, formatSize(duplicateReclaimable))
        }
        lifecycleScope.launch {
            val largeFilesSize = withContext(Dispatchers.IO) { estimateLargeFilesSize() }
            setActionSize(3, formatSize(largeFilesSize))
        }
    }

    /** Ước lượng nhanh tổng dung lượng các app cài lâu (>=30 ngày) chưa mở lại — cùng ngưỡng MIN_DAYS_UNUSED với UnusedAppsActivity. */
    private fun estimateUnusedAppsSize(): Long {
        return try {
            val usm = getSystemService(USAGE_STATS_SERVICE) as? android.app.usage.UsageStatsManager ?: return 0L
            val now = System.currentTimeMillis()
            val lookbackStart = now - java.util.concurrent.TimeUnit.DAYS.toMillis(365)
            val statsList = usm.queryUsageStats(android.app.usage.UsageStatsManager.INTERVAL_YEARLY, lookbackStart, now)
            val lastUsedByPackage = statsList.associateBy({ it.packageName }, { it.lastTimeUsed })
            val pm = packageManager
            val minUnusedMillis = java.util.concurrent.TimeUnit.DAYS.toMillis(30)

            pm.getInstalledApplications(android.content.pm.PackageManager.GET_META_DATA).sumOf { appInfo ->
                val isSystemApp = (appInfo.flags and android.content.pm.ApplicationInfo.FLAG_SYSTEM) != 0
                if (isSystemApp || appInfo.packageName == packageName) return@sumOf 0L
                val lastUsed = lastUsedByPackage[appInfo.packageName] ?: 0L
                val unusedFor = if (lastUsed <= 0L) Long.MAX_VALUE else now - lastUsed
                if (unusedFor < minUnusedMillis) return@sumOf 0L
                try { java.io.File(appInfo.sourceDir).length() } catch (e: Exception) { 0L }
            }
        } catch (e: Exception) {
            0L
        }
    ...
    /** Ước lượng nhanh dung lượng có thể giải phóng từ file trùng lặp — nhóm theo size (rẻ), không hash MD5 đầy đủ như DuplicateFilesActivity (chính xác tuyệt đối chỉ cần khi mở màn chi tiết). */
    private fun estimateDuplicateReclaimableSize(): Long {
        return try {
            val root = Environment.getExternalStorageDirectory() ?: return 0L
            val allFiles = mutableListOf<java.io.File>()
            fun collect(dir: java.io.File, depth: Int) {
                if (depth > 6) return
                val children = dir.listFiles() ?: return
                for (f in children) {
                    if (f.isDirectory) {
                        if (!f.name.startsWith(".")) collect(f, depth + 1)
                    } else {
                        allFiles.add(f)
                    }
                }
            ...
            collect(root, 0)
            allFiles.filter { it.length() > 4 * 1024L }
                .groupBy { it.length() }
                .values
                .filter { it.size >= 2 }
                .sumOf { group -> group.drop(1).sumOf { it.length() } }
        } catch (e: Exception) {
            0L
        }
    }

    /** Tổng dung lượng file > 25MB — cùng ngưỡng THRESHOLD_BYTES với LargeFilesActivity. */
    private fun estimateLargeFilesSize(): Long {
        return try {
            val root = Environment.getExternalStorageDirectory() ?: return 0L
            var total = 0L
            fun scan(dir: java.io.File, depth: Int) {
                ...
                for (f in children) {
                    ...
                    } else if (f.length() > 25 * 1024 * 1024L) {
                        total += f.length()
                    }
                }
            }
