...
class ClientFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        adapter = ConnectionAdapter(
            ...
            onEdit = { conn -> openEditConnection(conn) },
            onDelete = { conn ->
    ...
    private fun openEditConnection(conn: FtpConnectionProfile) {
        val intent = Intent(requireContext(), FtpConnectionActivity::class.java).apply {
            putExtra(FtpConnectionActivity.EXTRA_EDIT_CONNECTION_ID, conn.id)
        }
        startActivity(intent)
        activity?.let { ActivityTransitions.forward(it) }
    }
