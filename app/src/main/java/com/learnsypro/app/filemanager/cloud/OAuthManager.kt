 ...
 *
 * Redirect URI dùng chung: com.learnsypro.app://callback
 * (đã khai báo RedirectUriReceiverActivity trong AndroidManifest.xml)
 *
 * LƯU Ý: giá trị này PHẢI khớp tuyệt đối với "Redirect URI" khai báo trên
 * Dropbox App Console + Box Developer Console khi đăng ký app. Đổi chuỗi này
 * (kể cả chỉ đổi phần code, quên đổi manifest hoặc ngược lại) mà không cập
 * nhật tương ứng trên 2 console đó sẽ làm đăng nhập Dropbox/Box lỗi ngay bước
 * redirect quay lại app (trình duyệt không biết mở app nào).
 */
class OAuthManager(private val context: Context) {

    private val redirectUri = android.net.Uri.parse("com.learnsypro.app://callback")
    ...
    companion object {
        ...
        fun refreshAccessTokenBlocking(context: Context, provider: CloudProvider): String? {
            ...
            return try {
                // OkHttpClient() trần (mặc định) trước đây dùng timeout 10 giây/chiều — cùng
                // nguyên nhân gây lỗi mạng giả ở RetrofitFactory.client() đã ghi chú chi tiết
                // bên đó. Refresh token là bước ÂM THẦM chạy nền mỗi khi 1 request cloud dính
                // 401, nên nếu bước này tự timeout sớm, người dùng thấy y hệt lỗi "Đã có lỗi
                // xảy ra" dù access token đáng lẽ làm mới thành công nếu được chờ đủ lâu.
