...
object LogBus {
    ...
    // Đếm số dòng đã ghi kể từ lần trim gần nhất — tránh phải readLines() TOÀN BỘ
    // file mỗi lần persist() được gọi (trước đây làm vậy dù chỉ để so sánh với
    // MAX_FILE_LINES). Khi 1 nguồn nào đó log dồn dập liên tục trong thời gian ngắn
    // (điển hình: 1 service bị crash-loop do lỗi khởi động, tự restart lặp lại mỗi
    // ~1 giây và log lỗi mỗi lần) — đọc + ghi lại TOÀN BỘ file mỗi lần persist() dồn
    // dập hàng chục/trăm lần liên tiếp là I/O rất nặng, đủ để gây lag máy rõ rệt dù
    // bản thân file chưa bao giờ vượt quá MAX_FILE_LINES. Giờ chỉ đọc/ghi lại toàn
    // file khi thực sự cần trim (mỗi TRIM_CHECK_INTERVAL dòng), còn lại chỉ append.
    private const val TRIM_CHECK_INTERVAL = 100
    private var linesSinceTrim = 0
    ...
    fun error(msg: String, source: String = "APP", throwable: Throwable? = null) {
        val detail = throwable?.let { t ->
            // Nhiều thư viện (Commons Net, SSHJ, SMBJ...) bọc exception gốc bên trong 1 lớp
            // wrapper chung chung (ví dụ IOException("caught while copying")) — message của
            // lớp NGOÀI CÙNG gần như vô nghĩa để chẩn đoán (không nói rõ là timeout, mất kết
            // nối, hay lỗi gì khác). Đi xuống tận cause GỐC (sâu nhất trong chuỗi cause) mới
            // có message thật sự hữu ích (ví dụ "SocketTimeoutException: Read timed out").
            ...
            buildString {
                ...
                if (root !== t) {
                    // Vẫn giữ lại lớp bọc ngoài cùng để biết bối cảnh (ví dụ "trong lúc copy stream")
                    append(" (qua ").append(t::class.java.simpleName).append(")")
                }
                if (appFrame != null) {
